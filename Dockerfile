...
ARG ARG_GO_MODULE_NAME="github.com/epiphany-platform/m-azure-kubernetes-service"
...
RUN CGO_ENABLED=0 GOOS=linux go build -ldflags="-s -w $(govvv -flags -pkg $GO_MODULE_NAME/cmd -version $M_VERSION)" -x -o /m-azure-kubernetes-service $GO_MODULE_NAME
...
WORKDIR /workdir
ENTRYPOINT ["/workdir/m-azure-kubernetes-service"]

COPY --from=initializer /resources/ /resources/
COPY --from=builder /m-azure-kubernetes-service /workdir
